...
#define TIME_DELAY_BETWEEN_MEASUREMENTS ((1 * 60000) / portTICK_PERIOD_MS)
...
void vASensorControlTask(void *pvParameters)
{
	for (;;)
	{
		xEventGroupClearBits(_event_group_new_data, ((LIGHT_READY_BIT) | (CO2_READY_BIT) | (TEMPERATURE_HUMIDITY_READY_BIT)));

		xEventGroupClearBits(_event_group_measure, ((LIGHT_READY_BIT) | (CO2_READY_BIT) | (TEMPERATURE_HUMIDITY_READY_BIT)));
		//set bits to measure
		xEventGroupSetBits(_event_group_measure,
		((LIGHT_MEASURE_BIT) | (CO2_MEASURE_BIT) | (TEMPERATURE_HUMIDITY_MEASURE_BIT)));
		...
		((LIGHT_READY_BIT) | (CO2_READY_BIT) | (TEMPERATURE_HUMIDITY_READY_BIT)),
		pdTRUE,
		pdTRUE,
		portMAX_DELAY);
